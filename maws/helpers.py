...
def angle(array1, array2):
    return np.arccos(
        np.clip(
            np.dot(np.array(array1), np.array(array2))
            / (np.linalg.norm(np.array(array1)) * np.linalg.norm(np.array(array2))),
            -1,
            1,
        )
    )
...
def directed_angle(array1, array2, axis):
    ...
    return np.arctan2(
        np.dot(np.asarray(axis), np.cross(np.asarray(array1), np.asarray(array2))),
        np.dot(np.asarray(array1), np.asarray(array2)),
    )
