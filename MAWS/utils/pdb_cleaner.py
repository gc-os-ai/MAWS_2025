...
def non_std_residues(filename, pdb_df):
    ...
    residues = pdb_df['ResName'].str.upper()
    # 2. find anything not in our standard list
    nonstd = residues[~residues.isin(AMINO_ACIDS)].unique()
    # 3. check whether we actually got any back
    if nonstd.size > 0:
        return (filename, nonstd)
